...
GOLDEN_DIR = Path("tests/golden/api")
GOLDEN_DIR.mkdir(parents=True, exist_ok=True)


def _normalize_catalog(payload: dict[str, object]) -> dict[str, object]:
    rows = payload.get("rows") or []
    first = rows[0] if rows else {}
    return {"keys": sorted(payload.keys()), "row_keys": sorted(first.keys())}


def _normalize_rank(payload: dict[str, object]) -> dict[str, object]:
    offers = payload.get("offers") or []
    diagnostics = payload.get("provider_diagnostics") or []
    trace = payload.get("relaxation_steps") or []
    exclusion_breakdown = payload.get("exclusion_breakdown") or {}
...
def _normalize_invoice(payload: dict[str, object]) -> dict[str, object]:
    items = payload.get("line_items") or []
    opportunities = payload.get("savings_opportunities") or []
...
def main() -> None:
    catalog = run_browse_catalog(workload_type="llm").model_dump(mode="json")
    rank = run_rank_catalog(
    ...
    report = run_generate_report(
        ReportGenerateRequest(
            mode="catalog",
            title="API Report",
            catalog_ranking=run_rank_catalog(
                CatalogRankingRequest(
                    workload_type="speech_to_text",
                    allowed_providers=[],
                    unit_name=None,
    ...
    _write("catalog_llm", _normalize_catalog(catalog))
    _write("rank_catalog", _normalize_rank(rank))
    _write("plan_llm", _normalize_plan(llm))
    _write("report_generate", _normalize_report(report))
    _write("invoice_analyze", _normalize_invoice(invoice))
    print("Updated API contract golden snapshots.")
